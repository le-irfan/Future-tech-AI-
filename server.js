...
async function setupDatabase() {
  ...
  await pool.query(
    `CREATE TABLE IF NOT EXISTS comments(id TEXT PRIMARY KEY,post TEXT NOT NULL,name TEXT NOT NULL,comment TEXT NOT NULL,"createdAt" TIMESTAMPTZ NOT NULL,"updatedAt" TIMESTAMPTZ)`
  );
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/comments" && req.method === "POST")
    try {
      ...
      await pool.query(
        ...
        [c.id, c.post, c.name, c.comment, c.createdAt]
      );
  ...
  if (m && (req.method === "PUT" || req.method === "DELETE")) {
    ...
    try {
      if (req.method === "DELETE") {
        ...
        if (!r.rows.length) return sendJson(res, 404, { error: "Comment not found." });
        return sendJson(res, 200, r.rows[0]);
      }
      const i = JSON.parse(await body(req)),
        cur = await pool.query("SELECT name FROM comments WHERE id=$1", [id]);
      if (!cur.rows.length) return sendJson(res, 404, { error: "Comment not found." });
      const name = String(i.name || cur.rows[0].name).trim(),
        text = String(i.comment || "").trim();
  ...
  if (req.method === "GET") {
    ...
    if (safe.startsWith(ROOT) && fs.existsSync(safe) && fs.statSync(safe).isFile())
